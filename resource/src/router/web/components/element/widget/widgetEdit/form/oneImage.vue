<template>
    <div :class="$style['form-one-image']">
        ...
        <div :class="$style['image-hover-uploader']">
            <dropzone
                :cur-image="caseData[selectedIndex].imageHover[curLang]"
                :types="uploadType"
                :styles="dzStyle"
                :on-complete="(id, name, response) => { onImageUpload('imageHover', response.data.link); }"
                :on-remove="() => { onImageRemove('imageHover'); }"
            >
                <div :class="$style['icon-upload-cloud']" />
                <div :class="$style['uploader-text']">
                    <div>{{ $text('S_UPLOAD_IMG_HERE', { locale: memInfo.config.lang || 'zh-cn' }) }}</div>
...
.image-hover-uploader {
    composes: image-uploader;
    margin-right: 0;
}
