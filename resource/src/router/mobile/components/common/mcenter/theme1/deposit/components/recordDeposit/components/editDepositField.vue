                ...
                <div :class="$style['alert-body-wrap']">
                    <div :class="$style['deposit-info-table']">
                        <div :class="[$style['deposit-info-tr'], 'clearfix']">
                            <div :class="[$style['deposit-info-td'], $style['deposit-info-thead']]">{{ $text('S_ORDER_NUMBER', '订单号') }}</div>
                        ...
                        />
                        <div :class="[$style['deposit-info-tr'], 'clearfix']">
                            <div :class="[$style['deposit-info-td'], $style['deposit-info-thead']]">{{ $text('S_LAST_SUBMIT_TIME', '上次提交时间') }}</div>
                            <div :class="[$style['deposit-info-td'], { [$style['no-data']]: !depositData.submit_at }]">{{ depositData.submit_at || $text('S_NOT_ENTER_SHORT', '尚未提交') }}</div>
                        </div>
                    </div>
                    <div :class="$style['tip-time']">{{ $text('S_RECORD_TIP01', '将以您最后提交时的存款资料做审核') }}</div>
                    <div :class="$style['btn-wrap']">
                        <div :class="$style['cancel-btn']" @click="closeFuc(false)">{{ $text('S_CANCEL', '取消') }}</div>
                        <div :class="[$style['confirm-btn'], {[$style.disabled]: isSubmitDisabled}]" @click="saveDepositData">{{ $text('S_ENTER_SHORT', '提交') }}</div>
                    </div>
                </div>
            </div>
        </div>
...
<style lang="scss" src="../css/indexEditDepositField.module.scss" module></style>
