    ...
    <div :class="$style['add-bankcard']">
        <template v-if="addBankCardStep === 'one'">
            <div :class="$style['username-wrap']">
                ...
                <div :class="$style['input-wrap']">
                    <div v-if="memInfo.user.name" :class="[$style['input-wrap'],{[$style['has-name']] : memInfo.user.name} ]">
                        {{ username }}
                    </div>
                    <input
                        v-else
            ...
            <p v-if="!memInfo.user.name" :class="$style['username-remind']">为了您的资金能够迅速到账，请确保填写的姓名与银行卡的开户姓名一致</p>
        </template>
        <div :class="$style['card-info']">
            <template v-if="addBankCardStep === 'one'">
                <div :class="$style['info-item']">
                    ...
                    <div :class="$style['select-bank']" @click="isShowPop = true">
                        <span :class="{ [$style['select-active']] : currentBank }">{{ currentBank ? currentBank : '请选择银行' }}</span>
            ...
            <template v-if="addBankCardStep === 'two'">
                <p :class="[$style['error-msg'], {[$style['is-hide']] : !errorMsg}]">{{ errorMsg }}</p>
                <div :class="$style['info-item']">
            ...
            <div :class="$style['info-confirm']">
                <p :class="$style['confirm-remind']">
                    {{ addBankCardStep === 'one' ? '请认真校对银行卡号，卡号错误资金将无法到帐' : '手机号码将有助于您找回帐号，同时也是客服人员确认您身份的重要依据。' }}
                </p>
                <div
        ...
        <div v-if="isShowPop" :class="$style['pop-wrap']">
            ...
            <div :class="$style['pop-menu']">
                <div :class="$style['pop-title']">
                    <span @click="isShowPop = false">{{ $text('S_CANCEL', '取消') }}</span>
                    选择所属银行
                </div>
