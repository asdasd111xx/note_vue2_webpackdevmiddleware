<template>
  <div :class="[$style['date-picker-wrap'], 'clearfix']">
    <div :class="$style.wrap">
      ...
      >
        <swiper-slide v-for="value in yearData" :key="value">{{
          value
        }}</swiper-slide>
      </swiper>
    </div>
...
export default {
  ...
  watch: {
    monthData() {
      const index =
        this.currentYear === this.maxLimitYear ? this.monthData.length - 1 : 0;
      this.$nextTick(() => {
        this.$refs["month-picker"].$swiper.slideTo(index);
      });
    },
    dayData() {
      const index =
        this.currentMonth === this.maxLimitMonth ? this.dayData.length - 1 : 0;
  ...
  methods: {
    ...
    setMonth(index) {
      ...
      this.$emit(
        "update:date",
        new Date(this.currentYear, this.monthData[index] - 1, this.currentDay)
      );
    },
    setDay(index) {
