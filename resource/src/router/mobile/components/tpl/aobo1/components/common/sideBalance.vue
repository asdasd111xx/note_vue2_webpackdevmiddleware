  ...
  <div :class="[$style.balance, $style[`is-${$route.name.split('-')[0]}`]]">
    <div :class="$style.content">
      ...
      <span v-if="memInfo.user.bankrupt === '1'" :class="$style['prompt']">
        <img
          :class="$style['warning']"
          :src="$getCdnPath('/static/image/mobile/tpl/porn1/home/ic_alert.png')"
        />
      ...
      <div :class="[$style.top, 'clearfix']">
        <template v-if="!updating">
          ...
          <div :class="$style['total-amount']">
            {{ memBalance.total | commaFormat }} {{ memCurrency }}
          </div>
        </template>
      </div>
      <template v-if="!updating">
        ...
        <div :class="[$style.button, 'clearfix']">
          <!-- 額度轉換 -->
          <div :class="$style.trans" @click="toBalance">
            {{ $text("S_transfer") }}
          </div>
          <!-- 線上存款 -->
          <div :class="$style.deposite" @click="toDeposit">
            {{ $text("S_DEPOSIT") }}
          </div>
        </div>
        <!-- 更新餘額 -->
        <div :class="$style.update" @click="onUpdate">
          <i :class="$style.sync" />{{ $text("S_UPDATE_ALL_BALANCE") }}
        </div>
        <!-- 各平台餘額 -->
        <div :class="$style.wrap">
          <template v-for="(vendor, key) in memBalance.vendor">
            <div :key="key" :class="[$style.vendor, 'clearfix']">
              <div
                v-if="
                  memInfo.auto_transfer.enable &&
                    key === memInfo.auto_transfer.vendor
                "
                :class="$style.star"
              />
              <div :class="$style.title">{{ vendor.text }}</div>
              <div v-if="vendor.maintain" :class="$style.maintain">
                {{ $text("S_MAINTAIN") }}
              </div>
          ...
          <!-- 餘額提示 -->
          <div
            v-if="memInfo.auto_transfer.enable"
            :class="[$style.tips, 'clearfix']"
          >
...
.close {
  ...
  width: 20px;
  height: 20px;
  background: url("/static/image/mobile/tpl/brilliant/common/balance/btn_close.png")
    0 0 no-repeat;
  background-size: 100%;
}
...
.sync {
  ...
  height: 15px;
  margin-right: 8px;
  background: url("/static/image/mobile/tpl/porn1/home/updated_icon.png") 0 0
    no-repeat;
  background-size: 100%;
  vertical-align: middle;
}
...
.star {
  ...
  height: 9px;
  margin: 10px 3px 0 0;
  background: url("/static/image/mobile/tpl/brilliant/common/balance/icon_star.png")
    0 0 no-repeat;
  background-size: 100%;
}
