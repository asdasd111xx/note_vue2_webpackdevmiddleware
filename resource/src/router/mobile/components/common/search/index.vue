    ...
    <div :class="$style['search-wrap']">
        <div :class="[$style['search-header'], 'clearfix']">
            <input
                ...
                type="text"
            />
            <div :class="$style['search-cancel']" @click="updateSearchStatus()">{{ $text('S_CANCEL', '取消') }}</div>
        </div>
...
.search-input {
    ...
    height: 30px;
    padding-left: 42px;
    background: #EEE url('/static/image/_new/common/icon_search_n.png') no-repeat 3px center;
    border: none;
    border-radius: 5px;
