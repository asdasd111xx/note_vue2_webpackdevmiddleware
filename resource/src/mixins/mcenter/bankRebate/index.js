...
export default {
  ...
  computed: {
    ...
    list() {
      ...
      return this.rebateList.filter(
        (info, index) => index >= first && index < last
      );
    },
    messageText() {
    ...
    immediateData() {
      ...
      const resultData = this.caculateData.map(item => ({
        ...
        operateStatus:
          Number(item.rebate) >=
          Number(rebateInfoData[item.vip_config_id].min_rebate)
      }));
    ...
    realTimeRebateTotal() {
      ...
      if (this.immediateData && this.immediateData.length > 0) {
        const allTotal = this.immediateData.reduce(
          (item, nextItem) => Number(item) + Number(nextItem.rebate),
          0
        );
    ...
    realTimeValidBetTotal() {
      ...
      if (this.immediateData && this.immediateData.length > 0) {
        const allTotal = this.immediateData.reduce(
          (item, nextItem) => Number(item) + Number(nextItem.total),
          0
        );
        return allTotal ? allTotal.toFixed(2) : "--";
      }
    ...
    realTimePeriod() {
      ...
      const estDate =
        new Date(this.rebateInitData.event_end_at).getTime() +
        (new Date(this.rebateInitData.event_end_at).getTimezoneOffset() / 60) *
          3600000 -
        14400000;
      return `${this.EST(this.rebateInitData.event_start_at)}～${format(
        new Date(estDate),
        "HH:mm:ss"
      )}`;
    }
    ...
    popType(now, prev) {
      if (now === "" && prev === "rebate") {
        ...
        if (this.mcenterBankRebateInterval === "today") {
          ...
          this.mcenterBankRebateType === "" &&
          this.mcenterBankRebateInterval === ""
        ) {
          // 當實返派發有誤時，重新設定頁面資料
          this.init();
        }
  ...
  methods: {
        ...
        success: response => {
          ...
          this.pickDateList = {
            today: {
              ...
              startFullDate: Vue.moment(this.estToday).format(
                "YYYY/MM/DD 00:00:00"
              ),
              endFullDate: Vue.moment(this.estToday).format(
                "YYYY/MM/DD 23:59:59"
              )
            },
            yesterday: {
              startDate: Vue.moment(this.estToday)
                .add(-1, "days")
                .format("MM/DD"),
              endDate: Vue.moment(this.estToday)
                .add(-1, "days")
                .format("MM/DD"),
              startFullDate: Vue.moment(this.estToday)
                .add(-1, "days")
                .format("YYYY/MM/DD 00:00:00"),
              endFullDate: Vue.moment(this.estToday)
                .add(-1, "days")
      ...
      mcenter.bankRebateHistory({
        ...
        success: response => {
          ...
          for (let i = 0; i < this.rebateList.length; i += 1) {
            ...
            this.rebateList[i].rebate_done_at = this.EST(
              this.rebateList[i].rebate_done_at
            );

            if (this.rebateList[i].canceled) {
              this.rebateList[i].text = i18n.t("S_DISMISSA");
            } else if (
              this.rebateList[i].self &&
              this.rebateList[i].rebate_done_at == null
            ) {
    ...
    rebatelLock() {
      ...
      this.lockTimer = setInterval(() => {
        ...
        // 當分秒小於10時，數值補0
        const m =
          Math.floor((this.formatTime / 60) % 60) < 10
            ? `0${Math.floor((this.formatTime / 60) % 60)}`
            : Math.floor((this.formatTime / 60) % 60);
        const s =
          Math.floor(this.formatTime % 60) < 10
            ? `0${Math.floor(this.formatTime % 60)}`
            : Math.floor(this.formatTime % 60);
    ...
    popReceive(dataIndex) {
      ...
      this.actionSetSystemTime(() => {
        ...
        // 判斷跨日之後是否有重新按下試算
        const caculateTime = new Date(
          this.caculateData[dataIndex].now_at.replace("+0800", "+08:00")
        );
