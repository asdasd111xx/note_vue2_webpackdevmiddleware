<template>
  <div class="game-record">
    <div class="main-title clearfix">
      ...
      <span v-else class="title pointer" @click="currentType = 'main'">{{
        $t("S_MEM_GAME_RECORD")
      }}</span>
      <template v-if="currentType === 'second'">
        <span class="text"><span class="arrow">></span>{{ selectUser }}</span>
        <div class="main-btn" @click="currentType = 'main'">
          {{ $t("S_BACK_PAGE") }}
        </div>
      </template>
      <template v-if="currentType === 'detail'">
        <span class="text pointer" @click="currentType = 'second'"
          ><span class="arrow">></span> {{ selectUser }}</span
        >
        <span class="text"
          ><span class="arrow">></span
          >{{ getGameName(selectVendor, selectKind) }}</span
        >
        <div class="main-btn" @click="currentType = 'second'">
          {{ $t("S_BACK_PAGE") }}
        </div>
      </template>
    </div>
    ...
    <div v-if="currentType === 'main'" class="game-record-field clearfix">
      ...
      <div class="main-calendar-wrap clearfix">
        <span :title="$t('S_WAGER_TIME')" class="date-text">{{
          $t("S_WAGER_TIME")
        }}</span>
        <datePicker
          v-model="startTime"
      ...
      />
      <div
        class="main-btn inquire-btn"
        @click="inquire('main', '', '', '', 1, 'reset')"
      >
        {{ $t("S_INQUIRE") }}
      </div>
    </div>
    ...
    <template v-else>
        ...
        <div
          ...
          @click="
            inquire(
              ...
              selectKind,
              currentPage[currentType],
              selectSort[currentType]
            )
          "
        />
        <div
          :class="`sort-btn small ${
            sort[currentType] === 'asc' ? 'current' : ''
          }`"
          @click="
            inquire(
              currentType,
        ...
        >
          <thead class="agcenter-pc">
            <tr>
              ...
              <th v-if="currentType === 'main'" class="username">
                {{ $t("S_USER_NAME") }}
              </th>
              <th v-else-if="currentType === 'second'" class="platform">
                {{ $t("S_PLATFORM") }}
              </th>
              <th v-else-if="currentType === 'detail'" class="gametype">
                {{ $t("S_GAMETYPE2") }}
              </th>
              <th v-if="currentType === 'detail'" class="quantity">
                {{ $t("S_MA_NUM_BETS") }}
              </th>
              <th
                :class="`amount ${
                  selectSort[currentType] === 'bet' ? 'active' : ''
                }`"
                @click="
                  inquire(
                    currentType,
              ...
              >
                ...
                <icon
                  ...
                  :name="`${
                    sort[currentType] === 'desc' ? 'caret-down' : 'caret-up'
                  }`"
                  width="15"
                  height="15"
                />
                <icon v-else name="sort" width="15" height="15" />
              </th>
              <th
                :class="`valid ${
                  selectSort[currentType] === 'valid_bet' ? 'active' : ''
                }`"
                @click="
                  inquire(
                    currentType,
              ...
              <th
                :class="`result ${
                  selectSort[currentType] === 'payoff' ? 'active' : ''
                }`"
                @click="
                  inquire(
                    currentType,
              ...
              <th v-if="currentType !== 'detail'" class="view-next">
                {{ $t("S_OPERATE") }}
              </th>
            </tr>
          </thead>
          <tbody>
            <template v-for="(list, index) in allData[currentType]">
              <tr :key="index" class="agcenter-pc">
                ...
                <td v-if="currentType === 'main'" class="username">
                  {{ list.username }}
                </td>
                <td v-else-if="currentType === 'second'" class="platform">
                  {{ getGameName(list.vendor_name, list.kind) }}
                </td>
                <td v-else-if="currentType === 'detail'" class="gametype">
                  {{ list.text }}
                </td>
                <th v-if="currentType === 'detail'" class="quantity">
                  {{ list.counts }}
                ...
                <td :class="`result ${+list.payoff < 0 ? 'negative' : ''}`">
                  {{ list.payoff }}
                </td>
            ...
            <tr class="sub-total agcenter-pc">
              <td :colspan="currentType === 'detail' ? 3 : 2" class="title">
                {{
                  currentType === "main"
                    ? $t("S_PAGE_RESULT_SUM")
                    : $t("S_RESULT_SUM")
              ...
              <td
                :class="`result ${
                  +subTotalData[currentType].payoff < 0 ? 'negative' : ''
                }`"
              >
                {{ subTotalData[currentType].payoff }}
              </td>
              <td v-if="currentType !== 'detail'" class="view-next" />
            </tr>
            <table class="pad">
              ...
              <tr>
                ...
                <td class="amount content">
                  {{ subTotalData[currentType].bet }}
                </td>
              </tr>
              <tr>
                <td class="valid caption">{{ $t("S_VALID_BET") }}</td>
                <td class="valid content">
                  {{ subTotalData[currentType].valid_bet }}
                </td>
              </tr>
              <tr>
                <td class="result caption">{{ $t("S_PAY_OUT") }}</td>
            ...
            <tr v-if="currentType === 'main'" class="main-total agcenter-pc">
              ...
              <td
                :class="`result ${+mainTotalData.payoff < 0 ? 'negative' : ''}`"
              >
                {{ mainTotalData.payoff }}
              </td>
              <td class="view-next" />
            </tr>
            <table v-if="currentType === 'main'" class="pad">
              ...
              <tr>
                ...
                <td
                  :class="`result ${
                    +mainTotalData.payoff < 0 ? 'negative' : ''
                  } content`"
                >
                  {{ mainTotalData.payoff }}
                </td>
      ...
      <div class="page-wrap">
        <span
          :class="`page-first${
            currentPage[currentType] === 1 ? ' disable' : ''
          }`"
          @click="
        ...
        />
        <span
          :class="`page-pre${currentPage[currentType] === 1 ? ' disable' : ''}`"
          @click="
            currentPage[currentType] === 1
              ? ''
              : inquire(
        ...
        <span
          :class="`page-next${
            currentPage[currentType] === allPage[currentType] ? ' disable' : ''
          }`"
          @click="
            currentPage[currentType] === allPage[currentType]
              ? ''
              : inquire(
                  currentType,
                  selectUser,
...
export default {
  ...
  computed: {
    ...
    settleSwitch() {
      const isCustomizedBall =
        this.selectVendor === "csc" && this.selectKind === 1;
      const isBbinType =
        this.selectVendor === "bbin" &&
        (+this.selectKind === 1 || +this.selectKind === 4);

      return this.currentType === "detail" && (isCustomizedBall || isBbinType);
    },
    vendorList() {
      const games = Object.keys(this.gameData).reduce(
        (init, key) => [
          ...init,
          {
      ...
      return [{ value: "", text: this.$text("S_ALL") }, ...games];
    },
    currentSelect() {
      return {
  ...
  methods: {
    ...
    inquire(type, secondUser, detailVendor, detailKind, pageChange, action) {
      ...
      if (type === "detail") {
        ...
        if (
          (detailVendor === "bbin" &&
            (+detailKind === 1 || +detailKind === 4)) ||
          (detailVendor === "csc" && +detailKind === 1)
        ) {
          ...
          game.gameList(
            {
              ...
              success: (response) => {
                ...
                this.allData[type] = [...this.allData[type]];
              },
            },
            this.selectVendor
          );
