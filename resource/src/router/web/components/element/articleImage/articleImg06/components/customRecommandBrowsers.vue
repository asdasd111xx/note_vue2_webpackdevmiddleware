<template>
    <div class="custom-recommand-browsers-wrap">
        <!-- 文字區塊 title -->
        <article-inline
            :info="{ type: 'title', index: 11 }"
            :value="element.case.data[11].title[curLang]"
            :on-blur="onTitleBlur"
        ...
        <element-draggable :element="element">
            <template v-for="(item, index) in element.case.data">
                <element-wrap
                    v-if="index > 11 && index <= 17"
                    :key="`custom-recommand-browsers-${index}`"
                    :index="index"
                    :config="shadowConfig"
                    :element="element"
                    class="custom-recommand-browsers"
                >
...
.custom-recommand-browsers-wrap {
    width: 18%;
    float: left;
    padding-left: 5%;

    .custom-recommand-browsers {
        position: relative;
