<template>
    <div :class="getClass([`${!gameInfo.code && gameInfo.status ? 'game-item-activity' : 'game-item'}`, `game-type-${gameInfo.kind}`, `${!gameInfo.url && !gameInfo.code ? 'no-activity' : 'show'}`])" @click="onEnter">
        <div v-if="gameInfo.code && gameInfo.status < 4" :class="getClass(['game-active'])">
            <img :src="$getCdnPath(getActivityImg)" />
        </div>
        <!-- vendor 標示 -->
        <div v-if="showVendor" :class="getClass(['game-vendor'])">{{ vendorName }}</div>
        <!-- 最愛 -->
        <div :class="getClass(['icon-star-wrap'])" @click.stop="toggleFavorite">
            <div
                v-if="showFavor && loginStatus"
                :class="getClass(['icon-star', 'is-favorite'], { 'is-favorite': checkFavorite })"
            />
        </div>
        <!-- 遊戲圖片區塊 -->
        ...
        <!-- 遊戲標題 -->
        <div :class="getClass(['game-title', 'is-login'], { 'is-login': loginStatus })">{{ resultGameName }}</div>
        <!-- PC 版點擊按鈕開啟遊戲 -->
        <div v-if="showButton" :class="getClass(['btn-enter'])">{{ getBtnName }}</div>
    </div>
</template>
    ...
    computed: {
        ...
        */
        ...
        getActivityImg() {
            ...
            if (nowTheme === 'brilliant' || nowTheme === 'miller') {
