<template>
    <div :class="$style['simple-deposit-wrap']">
        <div :class="$style['header-wrap']">
            <span v-if="memInfo.config.domain_name" :class="$style['header-title']">{{ memInfo.config.domain_name[curLang] }}</span>
        ...
        <div :class="$style['content-wrap']">
            <div :class="$style['username-wrap']">
                <span :class="$style['member-title']">{{ $text('S_NAME', '会员帐号') }}</span>
            ...
            <li :class="$style['username-tip']">{{ $text('S_DEPOSIT_TIP01', '请确认您的帐号，款项存入后，即无法取消或变更帐号') }}</li>
            <preview-deposit v-if="!isShowDeposit" />
            <template v-else>
                <!-- 導覽選單 -->
                <div v-if="!submitStatus" :class="[$style['deposit-list-wrap'], 'clearfix']">
                    <div
                        ...
                        <div v-if="!curModeGroup.uri" :class="$style['bank-card-wrap']">
                            ...
                            <div :class="[$style['feature-wrap'], 'clearfix']">
                                ...
                                <div :class="$style['pay-mode-item']" @click="changeType('payMethod')">
                                    <span :class="$style['pay-main-title']">{{ curPayInfo.bank_name || curPayInfo.payment_method_name }} - {{ curPayInfo.payment_type_name }}</span>
                                </div>
                            </div>
                            <!-- 選擇銀行 -->
                            <div v-if="curPayInfo.banks && curPayInfo.banks.length > 0" :class="[$style['feature-wrap'], 'clearfix']">
                                ...
                                <div :class="$style['pay-mode-item']" @click="changeType('chagneBank')">
                                    <span :class="$style['pay-main-title']">{{ bankSelectValue.label }}</span>
                                </div>
                            </div>
                            <!-- 通道 -->
                            <div v-if="!isDepositAi && passRoad.length > 0" :class="[$style['feature-wrap'], 'clearfix']">
                                <span :class="$style['bank-card-title']">{{ $text('S_SELECT_PAY_PASS', '请选择支付通道') }}（{{ $text('S_PAY_PASS_TIP', '若通道无法存款，请点选其他通道') }}）</span>
                                <div :class="$style['bank-feature-wrap']" @click="changeType('payPass')">
                                    <div :class="$style['pay-mode-pass']">{{ curPassRoad.mainTitle }}</div>
                                </div>
                            </div>
                            <!-- 存款金額 -->
                            <div :class="[$style['feature-wrap'], 'clearfix']">
                                ...
                                <div v-if="isDepositAi || curPassRoad.is_recommend_amount" class="speed-money-wrap">
                                    <div
                            ...
                            <div v-if="curPayInfo.field.find(item => item.required) && [5, 6].includes(curPayInfo.payment_type_id)" :class="$style['speed-input-wrap']">
