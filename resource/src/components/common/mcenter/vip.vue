...
export default {
    ...
    methods: {
                            ...
                            common.image({
                                success: (res) => {
                                    const list = {
                                        ...
                                        levelUpTip: this.$text('S_VIP_FOREWORD', '您只需要')
                                    };
                                    const vipContent = [];
                                    ...
                                    if (+info.upgrade_valid_bet > 0) {
                                        vipContent.push(`${this.$text('S_VIP_BET_TIP', { replace: [{ target: '%s', value: `<span class="vip-value">${this.commaFormat(info.upgrade_valid_bet)}</span>` }] })}`);
                                    }
                                    if (+info.upgrade_deposit_times > 0) {
                                        vipContent.push(`${this.$text('S_VIP_DEPOSIT_TIMES', { replace: [{ target: '%s', value: `<span class="vip-value">${info.upgrade_deposit_times}</span>` }] })}`);
                                    }
                                    if (+info.upgrade_deposit_amount > 0) {
                                        vipContent.push(`${this.$text('S_VIP_DEPOSIT_AMOUNT', { replace: [{ target: '%s', value: `<span class="vip-value">${this.commaFormat(info.upgrade_deposit_amount)}</span>` }] })}`);
                                    }
                                    if (+info.upgrade_deposit_total > 0) {
                                        vipContent.push(`${this.$text('S_VIP_DEPOSIT_TOTAL', { replace: [{ target: '%s', value: `<span class="vip-value">${this.commaFormat(info.upgrade_deposit_total)}</span>` }] })}`);
                                    }

                                    list.levelUpTip += vipContent.join('，');
                                    list.levelUpTip += `${this.$text('S_VIP_LEVEL_ALIAS', { replace: [{ target: '%s', value: `<span class="vip-value">${info.next_level_alias}</span>` }] })}`;

                                    if (!list.is_lock) {
                                        this.$set(this.userVipInfo, info.config_id, list);
                                    }
        ...
        getVipLevel(info) {
            ...
            return mcenter.vipLevelList({
                ...
                success: (result) => {
                    ...
                    const showIndex = result.ret.map((item) => item.id).indexOf(info.now_level_id);

                    if (showIndex !== 0) {
                        const resultVipList = result.ret.slice(showIndex, result.ret.length);
                        this.$set(this.vipLevelList, info.config_id, resultVipList);
                    } else {
                        this.$set(this.vipLevelList, info.config_id, result.ret);
                    }
                        ...
                        common.image({
                            success: (res) => {
                                ...
                                if (info.now_level_id === val.id) {
                                    this.$set(this.nowTypeLevel, info.config_id, index);
                                }
                            }
                        }, val.image.image_id);
                    });
        ...
        previewLevel(level, status) {
            ...
            if (status === 'next') {
                this.$set(this.nowTypeLevel, level.config_id, this.nowTypeLevel[level.config_id] + 1);
            } else if (status === 'pre') {
                this.$set(this.nowTypeLevel, level.config_id, this.nowTypeLevel[level.config_id] - 1);
