<template>
    <div>
        <!-- vendor 標示 -->
        <div v-if="listInfo.kind === 3 || listInfo.kind === 5 || listInfo.kind === 6" class="vendor-name">{{ getVendorName }}</div>
        <!-- 遊戲圖片區塊 -->
...
export default {
    ...
    computed: {
        ...
        getVendorName() {
            ...
            const resultText = transList[vendor] ? transList[vendor] : vendor;
...
<style lang="scss" src="../css/listItem.scss" scoped></style>
