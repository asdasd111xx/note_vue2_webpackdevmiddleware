            ...
            <div class="bank-balance-tran">
                ...
                <div class="balance-detail-inner">
                    <div class="balance-total-price clearfix">
                        ...
                        <span :class="`icon-refresh ${balanceLock ? 'disable' : ''}`" @click="getBalanceAll('lockStatus')">
                            <icon name="sync" width="12" />
                        </span>
                        <div :class="`main-btn btn-back-account ${balanceBackLock ? 'disable' : ''}`" @click="balanceBack()">{{ balanceBackLock ? '' : $t('S_ONE_CLICK_TO_ACCOUNT') }}</div>
                    </div>
                    <div class="main-account-price">{{ $t('S_MAIN_BALANCE') }}： <span class="price">{{ membalance.vendor.default ? membalance.vendor.default.amount : '' }}</span></div>
                    ...
                    <div class="main-balance-inner">
                            ...
                            <div class="balance-field-wrap clearfix">
                                ...
                                <div class="balance-field money">
                                    ...
                                    <input
                                        v-model="money"
                                        type="text"
                                        class="main-input money"
                                    />
                                </div>
                                <div :class="`main-btn tran-btn ${btnLock ? 'lock' : ''}`" @click="balanceTran()">{{ $t('S_CONFIRM_TRANS_BALANCE') }}</div>
                            </div>
                        </div>
                        <div class="balance-field-wrap clearfix">
                            <div
                                ...
                                :key="index"
                                :class="['balance-info', { right: index % 2 === 1 }, 'clearfix']"
                            >
                                <span class="balance-info-text">
                                    <span v-if="key === recentlyData.vendor && isAutotransfer" class="balance-recently-opened">*</span>
                                    {{ item.text }}
                                </span>
                                <span class="balance-info-money">{{ item.amount }}</span>
                                <span v-if="item.maintain" class="balance-info-maintain tooltip">
                                    ({{ $t('S_MAINTAIN') }})
                                    <span class="tooltiptext">
                                        {{ $t('S_TIME_OF_EAST_US') }}：<br />
                                        {{ item.maintain.etc_start_at }} - {{ item.maintain.etc_end_at }}<br />
                        ...
                        <p v-if="recentlyData.vendor && recentlyData.vendor != '--' && isAutotransfer" class="balance-recently-opened-notice">{{ $t('S_FINALLY_OPENED_PLATFORM') }}</p>
                    </div>
                </div>
