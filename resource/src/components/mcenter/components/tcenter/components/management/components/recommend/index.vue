<template>
    <div :class="mainClass">
        <div :class="[$style['code-wrap'], 'clearfix']">
            <div :class="$style.title">{{ $text('S_PROMOTION_CODE', '推广代码') }}</div>
        ...
        <div v-if="agentLink" :class="[$style['promote-wrap'], 'clearfix']">
            ...
            <div :class="$style['button-wrap']">
                <div class="download-qrcode" @click="onQrcodeOpen">
                    ...
                    <span :class="$style['button-text']">{{ $text('S_DOWNLOAD_QRCODE', '下载二维码') }}</span>
                </div>
                <div class="copy-link" @click="onCopy('link')">
                    <span :class="[$style.icon, $style['icon-copy']]" />
                    <span :class="$style['button-text']">{{ $text('S_COPY_PROMOTION_LINK', '复制推广连结') }}</span>
                </div>
                <div class="copy-code" @click="onCopy('code')">
                    <span :class="[$style.icon, $style['icon-copy']]" />
                    <span :class="$style['button-text']">{{ $text('S_COPY_PROMOTION_CODE', '复制推广代码') }}</span>
                </div>
            </div>
            <div
        ...
        <transition name="slide-down">
            <div v-if="isShow" :class="$style['register-wrap']">
                ...
                >
                    <!-- 欄位名稱 -->
                    <div :class="$style['input-title']">{{ $text(allText[key].placeholder) }}</div>
                    <!-- 輸入框 -->
                    <div :class="[$style[key], $style.placeholder, { [$style.error]: allText[key].error }]">
                        ...
                        <div
                            ...
                            :class="[$style['btn-show-password'], { [$style.active]: allText[key].type === 'text' }]"
