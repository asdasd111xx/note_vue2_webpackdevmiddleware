        ...
        <template>
          <div class="content">
            ...
            <div class="rebate-value">
              {{ computeState ? receiveEntryData.start_at : data.start_at }} ~
              {{ computeState ? receiveEntryData.end_at : data.end_at }}
            </div>
            <template v-if="computeState">
              ...
              <div class="rebate-entry-button-wrap">
                ...
                <a id="button-entry-close" href="###" @click="close()">
                  {{ $t("S_CLOSE") }}
                </a>
              </div>
            </template>
...
export default {
  components: {
    eleLoading: () =>
      import(
        /* webpackChunkName: 'eleLoading' */ "../../../../../../tpl/common/element/loading/circle_text"
      )
  },
  props: {
    data: {
  ...
  methods: {
      ...
      mcenter.bankRebateReceiveEntry(
        {
          success: response => {
            ...
            this.receiveEntryData.start_at = EST(
              this.receiveEntryData.start_at
            );
    ...
    ...mapActions(["actionSetPop", "actionSetMcenterBankRebate"])
  }
};
</script>
