  ...
  <div slot="content" :class="$style['content-wrap']">
    ...
    <div :class="[$style.wrap, 'clearfix']">
      ...
      <div :class="$style.block">
        <div :class="$style.title">{{ $text("S_KAKAOTALK") }}</div>
        <div :class="$style['input-wrap']">
          <input
            ref="input"
            v-model="value"
            :value="value"
            :placeholder="`请输入${$text('S_KAKAOTALK')}`"
            :class="$style.input"
            :maxlength="100"
            type="text"
            @input="onInput"
...
export default {
  ...
  created() {
    if (this.memInfo.user.kakaotalk) {
