<template>
  <transition name="fade">
    <div class="pop-note-warp pop-class">
      ...
      <div
        :class="['content', isException && path[0] === 'mobile' ? 'left' : '']"
      >
        <template>
          <p>
...
export default {
  ...
  computed: {
    ...
    isMobilePath() {
      if (
        (this.curLang === "zh-tw" || this.curLang === "zh-cn") &&
        this.path[0] === "mobile" &&
      ...
      return (
        (this.curLang === "ja" || this.curLang === "en") &&
        this.path[0] === "mobile"
      );
    },
    noteMsg() {
      return [
        ...
        },
        {
          ...
          isShow: this.isException
            ? false
            : !this.agentInfo.user.has_withdraw_password
        },
        {
  ...
  created() {
    ...
    const hasWithdrawPassword = this.isException
      ? false
      : !this.agentInfo.user.has_withdraw_password;
  ...
  methods: {
    ...
    ...mapActions(["actionSetPop", "actionChangePage"])
  }
};
</script>
