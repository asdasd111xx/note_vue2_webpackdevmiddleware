import {
  ...
  API_MCENTER_DEPOSIT_RECORD,
  API_MCENTER_DEPOSIT_SINGLE
} from "@/config/api";
...
export default {
    ...
    return {
      recordHeader: [
        ...
        },
        {
          ...
          value: this.$text("S_REAL_ENTER", "实际到帐"),
  ...
  computed: {
    ...
    recordText() {
      return this.$text("S_RECORD_TEXT", {
        replace: [
          {
            target: "%s",
            value: `<span>${this.paramsData.first_result + 1}</span>`
          },
          {
            target: "%s",
            value: `<span>${this.paramsData.first_result + 10}</span>`
          },
    ...
    requiredFields() {
      let ret = [];
      this.paymentGroups.find(payGroup =>
        payGroup.payment_group_content.find(content => {
          if (
            content.payment_type_id === this.singleDeposit.type_id &&
            content.payment_method_id === this.singleDeposit.method_id
          ) {
            ret = content.field;
            return true;
          }
