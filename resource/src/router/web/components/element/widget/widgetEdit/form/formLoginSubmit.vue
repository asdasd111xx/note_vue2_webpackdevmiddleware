<template>
    <div :class="$style['form-login-submit']">
        <slot name="side-control" />
        <slot name="field-z-index" />
        <slot name="field-vector" />
        <slot name="field-box" />
        <div :class="$style['switch-wrap']">
            <field-checkbox :title="$text('S_USE_BG', '使用背景图')" :value.sync="useImage" />
        </div>
        <div :class="$style['setting-wrap']">
            <div class="field-wrap">
                <div :class="[$style['field-title']]">{{ $text('S_FONT', '文字') }}</div>
                <input
                    v-model="btnText"
                    :class="$style.input"
                    type="text"
            ...
            <field-select
                :class="$style['select']"
                :title="$text('S_FONT_SIZE', '文字大小')"
                :options="fontSizeOptions"
                :value.sync="fontSize"
            />
            <field-select
                :class="$style['select']"
                :title="$text('S_INPUT_BORDER_SETTING', '输入框圆角设定')"
        ...
        <div :class="$style['setting-wrap']">
            <color-picker
                ...
                :title="$text('S_INPUT_COLOR', '输入框文字颜色')"
                :value-color.sync="fontColor"
            />
            <color-picker
                v-if="useImage !== 'Y'"
                :class="$style['color-picker']"
                :title="$text('S_INPUT_BG_COLOR', '输入框背景颜色')"
...
export default {
    ...
    computed: {
        ...
        btnText: {
            get() { return this.caseData[this.selectedIndex].btnText[this.curLang]; },
            set(value) {
                const cloneData = cloneDeep(this.caseData);

                cloneData[this.selectedIndex].btnText[this.curLang] = value;
