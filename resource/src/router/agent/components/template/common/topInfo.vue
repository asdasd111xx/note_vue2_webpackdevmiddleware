...
export default {
  ...
  computed: {
    ...
    contentTitle() {
      ...
      const name =
        this.agentUserConfig.config &&
        this.agentUserConfig.config.alias.display &&
        this.agentInfo.user.show_alias
          ? this.agentInfo.user.alias
          : this.agentInfo.user.username;
      let result = contentTitle.replace(
        "%s",
        ...
        result = contentTitle.replace(
          ...
          `<span class="${this.$style.account}"> ${name}</span>
                    <span class="${this.$style["change-name-wrap"]}">
                        <img class="${
                          this.$style["change-name-img"]
                        }" src="${this.$getCdnPath(
            "/static/image/porn1/agent/mcenter/change_icon.png"
          )}" />
                    </span>`
        );
      }
  ...
  mounted() {
    $(this.$refs["message-text"]).on("click", "span", () =>
      this.$router.push({
        path: isMobile() ? "/mobile/agcenter/message" : "/agent/infoMessage"
      })
    );
    $(this.$refs["content-text"]).on(
      "click",
      `.${this.$style["change-name-wrap"]}`,
      this.changeNickname
    );
  },
  beforeDestroy() {
    $(this.$refs["message-text"]).off("click", "span", () =>
      this.$router.push({
        path: isMobile() ? "/mobile/agcenter/message" : "/agent/infoMessage"
      })
    );
    $(this.$refs["content-text"]).off(
      "click",
      `.${this.$style["change-name-wrap"]}`,
      this.changeNickname
    );
  ...
  methods: {
    ...
    estClock() {
      ...
      this.estTimer = setInterval(() => {
        this.estTimeNow = Vue.moment(this.estTimeNow)
          .add(1, "seconds")
    ...
    changeNickname() {
      ...
      ajax({
        method: "put",
        url: this.agentInfo.user.show_alias
          ? API_AGENT_DISABLE_ALIAS
          : API_AGENT_ENABLE_ALIAS,
        errorAlert: false
      }).then(() => {
...
<style
  src="../../../css/common/topInfo.module.scss"
  lang="scss"
  module="$styleDefault"
></style>
<style
  src="../../../css/template/theme3/topInfo.module.scss"
  lang="scss"
  module="$styleTheme3"
></style>
<style
  src="../../../css/template/miller/topInfo.module.scss"
  lang="scss"
  module="$styleMiller"
></style>
<style
  src="../../../css/template/purple/topInfo.module.scss"
  lang="scss"
  module="$stylePurple"
></style>
