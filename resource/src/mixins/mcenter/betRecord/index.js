...
import {
  API_MCENTER_BETRECORD_TOTAL,
  API_MCENTER_BETRECORD_ALL
} from "@/config/api";
...
export default {
  ...
  created() {
    ...
    this.getAllGameData();
    this.limitDays = new Date(
      Vue.moment(this.estToday).add(this.initLimiDay, "days")
    );
  ...
  methods: {
        ...
        success: response => {
          ...
          Object.keys(temp).forEach(index => {
            tempData[index] = {
              ...
              money:
                +temp[index].total !== 0
                  ? Number(temp[index].total).toFixed(2)
                  : +temp[index].total,
              text: textObj[index]
            };
          });
    ...
    getAllGameData() {
      Object.keys(this.gameData).forEach(index => {
        ...
        if (
          this.gameData[index].switch === "Y" &&
          this.options.map(item => item.value).indexOf(vendor) === -1
        ) {
      ...
      return ajax({
        ...
        success: response => {
          ...
          for (let index = 0; index < this.mainListData.length; index += 1) {
            this.mainTotal.bet = this.accAdd(
              this.mainTotal.bet,
              this.mainListData[index].bet
            );
            this.mainTotal.valid_bet = this.accAdd(
              this.mainTotal.valid_bet,
              this.mainListData[index].valid_bet
            );
            this.mainTotal.payoff = this.accAdd(
              this.mainTotal.payoff,
              this.mainListData[index].payoff
            );
          }
