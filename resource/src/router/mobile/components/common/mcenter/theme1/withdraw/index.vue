  ...
  <div v-if="mobileCheck" :class="[$style['bank-withdraw'], colorClass]">
    ...
    <div v-if="!isSerial && isShow">
      <div :class="[$style['withdraw-title-wrap'], 'clearfix']">
        <span
          :class="[
            $style['withdraw-title'],
            { [$style['is-current']]: curTitleTab === 'withdrawMethod' }
          ]"
          title="提现方式"
        ...
        <span
          :class="[
            $style['withdraw-title'],
            { [$style['is-current']]: curTitleTab === 'withdrawRecord' }
          ]"
          title="提现记录"
      ...
      <div :class="[$style['main-money-wrap'], 'clearfix']">
        <div :class="$style['main-money']">
          <div :class="$style['main-title']">
            {{ $text("S_MAIN_BALANCE", "主帐户余额") }}
          </div>
          <div :class="$style['money-text']">
            {{ memCurrency }} {{ withdrawData.cash.balance }}
          </div>
        </div>
        <div
        ...
        <div :class="$style['icon-wrap']" @click="updateAmount">
          <icon name="sync-alt" width="15" height="15" />
        </div>
      </div>
      <template v-if="curTitleTab === 'withdrawMethod'">
        <div :class="[$style['withdraw-money-info'], 'clearfix']">
          <div :class="$style['withdraw-money-title']">
            ...
            >
              <img
                src="/static/image/mobile/mcenter/btn_description_white.png"
              />
            </div>
          </div>
          <div :class="$style['withdraw-money-text']">
        ...
        <template v-if="!thirdUrl">
          ...
          <div :class="[$style['withdraw-feature-wrap'], 'clearfix']">
            <div :class="$style['withdraw-input-wrap']">
              <div :class="$style['withdraw-input-title']">
                {{ withdrawMoneyTip.text }}
              </div>
              <div :class="$style['input-wrap']">
                <input
                  ...
                  placeholder="请输入提现金额"
                  @input="
                    withdrawValue = $event.target.value.replace(/[^\d]/g, '')
                  "
                />
              </div>
            </div>
            <template v-if="!memInfo.config.internal_audit">
              <div :class="[$style['withdraw-info'], 'clearfix']">
                <div :class="$style['withdraw-info-title']">
                  ...
                  <div
                    ...
                    @click.stop="
                      alertMsg(
                        $text('S_WITHDRAW_DEDUCTION_TIP', {
                          replace: [
                            {
                              target: '%s',
                              value: withdrawData.payment_charge.ret.free_period
                            },
                            {
                              target: '%s',
                              value: withdrawData.payment_charge.ret.free_count
                            }
                          ]
                        })
                      )
                  ...
                  >
                    <img
                      src="/static/image/mobile/mcenter/btn_description_n.png"
                    />
                  </div>
                </div>
                <div :class="$style['withdraw-info-money']">
                  {{
                    +withdrawData.audit.total.fee
                      ? `-${withdrawData.audit.total.fee}`
                      : "--"
                  }}
                </div>
              ...
              <div :class="[$style['withdraw-info'], 'clearfix']">
                ...
                <div :class="$style['withdraw-info-money']">
                  {{
                    +withdrawData.audit.total.deduction
                      ? `-${withdrawData.audit.total.deduction}`
                      : "--"
                  }}
                </div>
          ...
          <div
            v-if="!memInfo.config.internal_audit"
            :class="[$style['water-wrap'], 'clearfix']"
          >
            <span :class="$style['details-tip']">{{ serialTip }}</span>
            <span :class="$style['details-btn']" @click="serialClose(true)">{{
              $text("S_VIEW_DETAILS", "查看详情")
            }}</span>
          </div>
          <!-- 取款帳號區塊 -->
          <div
            v-if="memInfo.config.withdraw === '迅付'"
            :class="[$style['withdraw-feature-wrap'], 'clearfix']"
          >
            <div
              :class="[
                $style['withdraw-info'],
            ...
            <div :class="[$style['withdraw-info'], 'clearfix']">
              ...
              <div :class="$style['input-wrap']">
                <input
                  ...
                  type="password"
                  @input="
                    withdrawPwd = $event.target.value.replace(/[^\d]/g, '')
                  "
                />
              </div>
            </div>
          ...
          <div
            v-if="
              +resultWithdrawAccount.offer_percent &&
                memInfo.config.withdraw === '迅付'
            "
            :class="$style['withdraw-tip']"
          >
            {{ cgpayPromotionText }}
          </div>
          <div
            v-if="
              !memInfo.config.internal_audit &&
                (+withdrawData.audit.total.deduction ||
                  +withdrawData.audit.total.fee)
            "
            :class="$style['withdraw-checkbox-wrap']"
          >
            <input
              ...
              type="checkbox"
            />
            <div :class="$style['input-tip']">
              我同意扣除上述费用，并经财务审核后才可提现 !
            </div>
          </div>
...
export default {
  components: {
    withdrawRecord: () =>
      import(
        /* webpackChunkName: 'withdrawRecord' */ "./components/withdrawRecord"
      ),
    serialNumber: () =>
      import(
        /* webpackChunkName: 'serialNumber' */ "./components/serialNumber"
      ),
    withdrawAlert: () =>
      import(
  ...
  methods: {
    ...
    enterWithdrawOrder() {
      if (
        !this.isCheckWithdraw &&
        +this.withdrawData.audit.total.deduction &&
        !this.memInfo.config.internal_audit
      ) {
    ...
    changeSelectAccountValue(val) {
      const index = this.resultSelectWithdraw
        .map(item => item.value)
        .indexOf(val);
