<template>
    <div v-if="fieldsData.length > 0" :class="['normal-page', $style['join-wrap'], 'clearfix']">
        <div :class="$style.join">
            ...
            <div :class="$style['join-content']">
                ...
                >
                    ...
                    >
                        <span :class="$style['field-text']">{{ $t(joinAgentInfo[field.key].text) }}</span>
                        <span v-if="joinAgentInfo[field.key].isRequired" :class="$style['join-star']">*</span>
                    ...
                    <div :class="$style['field-right']">
                        ...
                        <v-select
                            ...
                            :class="num === 4 ? $style['join-input-withdraw-last'] : $style['join-input-withdraw']"
                        ...
                        <template v-else-if="field.key === 'phone'">
                            ...
                            <input
                                ...
                                :class="[$style['join-input'], $style['join-input-phone'], field.key]"
                                :name="field.key"
                        ...
                        <template v-else-if="field.key === 'captcha_text'">
                            ...
                            <div :class="$style['redo-wrap']" @click="clickCaptcha">
                                <icon :class="[$style['captcha-reform'], { [$style['is-mobile']]: isMobile }]" name="redo-alt" />
                            </div>
                            <img
                                v-if="captchaImg"
                                :src="captchaImg"
                        ...
                        <template v-else-if="field.key === 'password' || field.key === 'confirm_password'">
                            <input
                        ...
                        <input
                            ...
                            :ref="`field-${field.key}`"
                            v-model="allValue[field.key]"
                            :class="[$style['join-input'], { [$style['join-input-email']]: joinAgentInfo[field.key].hasVerify }, field.key]"
                            :name="field.key"
                            :placeholder="field.content.note1"
                            type="text"
                            @input="verification(field.key)"
                        ...
                        <template v-if="joinAgentInfo[field.key].hasVerify">
                            <div v-if="isVerified[field.key] && allValue[field.key] === oldValue[field.key]" :class="$style['icon-verify']">
                                <icon
                                    name="check-circle"
                                    width="16"
                                    height="16"
                            ...
                            <div
                                v-else
                                :class="[$style['btn-verify'], { [$style.disabled]: !allValue[field.key] || allTip[field.key] || (field.key === 'phone' && !countryCode) }]"
                                @click="getVerify(field.key, !allValue[field.key] || (field.key === 'phone' && !countryCode))"
                            >
                                {{ $text('S_SEND_CHECK_CODE', '发送验证码') }}
                            </div>
                        </template>
                        <!-- eslint-disable vue/no-v-html -->
                        <div :class="allTip[field.key] ? $style['join-tip-show'] : $style['join-tip']" v-html="allTip[field.key]" />
            ...
            <!-- 滑動驗證 -->
            <div v-if="memInfo.config.register_captcha_type === constants.CAPTCHA_SLIDE" :class="$style['join-content']">
                <slide-verification
...
export default {
    ...
    computed: {
        ...
        fieldsData() {
            return this.registerData.filter((field) => {
                if (field.key === 'captcha_text' && this.memInfo.config.register_captcha_type !== constants.CAPTCHA_GRAPH) {
                    return false;
                }

                return this.joinAgentInfo[field.key] && this.joinAgentInfo[field.key].show;
            });
        },
        $style() {
            if (this.theme) {
            ...
            return this.registerData
                ...
                .every((field) => {
                    ...
                    if (this.joinAgentInfo[field.key].isRequired) {
                        if (this.joinAgentInfo[field.key].type !== 'select' && field.key !== 'birthday') {
                        ...
                        if (field.key === 'phone') {
                            return this.joinAgentInfo[field.key].hasVerify && this.countryCode;
                        }
        ...
        agent.joinConfig({
            success: ({ result, ret }) => {
                ...
                Object.keys(this.joinAgentInfo).forEach((key) => {
                    if (key === 'captcha_text' && this.memInfo.config.register_captcha_type !== constants.CAPTCHA_GRAPH) {
    ...
    methods: {
        ...
        verification(key, inputValue = null) {
            if (inputValue !== null) {
                this.allValue[key] = key === 'captcha_text' ? inputValue.replace(/[^A-Za-z0-9]/g, '') : inputValue;
            }
         ...
         */
        submitCheck() {
            if (this.memInfo.config.register_captcha_type === constants.CAPTCHA_PUZZLE) {
            ...
            data.forEach((field) => {
                if (this.joinAgentInfo[field.key].show) {
                    ...
                    || (this.joinAgentInfo[field.key].isRequired
                        ...
                        || ((this.joinAgentInfo[field.key].type !== 'select') && this.allValue[field.key].replace(/(^\s*)|(\s*$)/g, '') === '')
                        || (field.key === 'gender' && +this.allValue[field.key] === 0)
                        || (field.key === 'withdraw_password' && this.allValue.withdraw_password.length < 4)
                        || (field.key === 'phone' && this.joinAgentInfo[field.key].hasVerify && !this.countryCode)
                        )
                    )
                    ) {
                        this.checkFail = true;
        ...
        joinSubmit(captchaInfo) {
            ...
            if (this.memInfo.config.register_captcha_type === constants.CAPTCHA_SLIDE
            ...
            agent.join({
                ...
                const onlyCaptchaError = Object.keys(response.errors).length === 1 && response.errors.captcha_text;
                if (this.memInfo.config.register_captcha_type === constants.CAPTCHA_GRAPH) {
                    if (onlyCaptchaError) {
                        this.clickCaptcha();
                        return;
