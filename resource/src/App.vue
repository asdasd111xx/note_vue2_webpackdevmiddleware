  ...
  methods: {
    ...
    setAnalyticsCode() {
      ...
      window.__lc = {
        visitor: {
          name:
            this.memInfo.user.username === "unknown"
              ? ""
    ...
    connectYaboWSV2() {
      ...
      try {
        ...
        window.YABO_SOCKETV2.onmessage = e => {
          ...
          window.YABO_SOCKETV2_ID = this.AESDecrypt(
            jsonData.Res,
            this.RSAdecrypt(jsonData.Key)
          ).SocketId;
    ...
    AESencrypt(word) {
      ...
      let encrypted = CryptoJS.AES.encrypt(srcs, key, {
        iv: CryptoJS.enc.Utf8.parse(this.IVKey),
    ...
    setGoogleAnalytics() {
      if (window.dataLayer && !this.setGtmCode) {
        ...
        window.dataLayer.push({
          ...
          ga_uid:
            !this.memInfo.user || !this.memInfo.user.id
              ? undefined
              : (+this.memInfo.user.id).toString(24),
          ga_user_type: undefined,
          ga_hall_id: 3820325,
          ga_game_group: undefined,
          ga_game_id: undefined,
          ga_domain_id:
            !this.memInfo.user || !this.memInfo.user.domain
              ? undefined
              : this.memInfo.user.domain
        });
      }
