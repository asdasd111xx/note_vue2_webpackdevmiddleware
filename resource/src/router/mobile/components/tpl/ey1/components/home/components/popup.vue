        ...
        <div :class="$style['modal-wrap']">
            <div :class="$style['modal-content']">
                ...
                <div :class="[$style['no-remind'], 'clearfix']">
                    <span :class="[$style['checkbox-circle'], {[$style.active] : isTick}]" @click="isTick=!isTick" />
                    <span :class="$style['checkbox-text']">不再提示</span>
                </div>
            </div>
            <div class="clearfix">
                <div :class="$style['modal-button']" @click="closePop">知道了</div>
                <div :class="$style['modal-button']" @click="closePop('mcenter')">查看详情</div>
            </div>
        </div>
    </div>
</template>
...
.checkbox-circle {
    ...
    height: 16px;
    margin-right: 4px;
    background: url('/static/image/_new/common/special_uncheck.png') 0 0/contain no-repeat;
