<template>
    <div class="game-record">
        ...
        <div v-if="currentType === 'detail' && selectVendor === 'bbin' && (+(selectKind) === 1 || +(selectKind) === 4)" class="settle-wrap clearfix">
...
export default {
    ...
    data() {
        return {
            ...
            currentType: 'main',
            vendorList: [{ value: '', text: this.$t('S_ALL') }],
            inputVendor: '',
            inputUser: '',
            startTime: '',
            endTime: '',
    ...
    created() {
        this.limitDays = new Date(Vue.moment(this.estToday).add(-60, 'days'));
        ...
        Object.keys(this.gameData).forEach((index) => {
            ...
            if (this.gameData[index].switch === 'Y' && this.vendorList.map((item) => item.value).indexOf(vendor) === -1) {
                this.vendorList.push({ value: vendor, text: this.getGameName(vendor) });
            }
        });
    },
    methods: {
        ...
        inquire(type, secondUser, detailVendor, detailKind, pageChange, action) {
            ...
            if (type === 'detail') {
                ...
                if (detailVendor === 'bbin' && (+(detailKind) === 1 || +(detailKind) === 4)) {
