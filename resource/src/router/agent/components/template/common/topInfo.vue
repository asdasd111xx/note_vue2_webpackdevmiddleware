<template>
    <div :class="[$style['agcenter-content-top'], $style[agentLang]]">
...
import { API_AGENT_USER_CONFIG, API_AGENT_DISABLE_ALIAS, API_AGENT_ENABLE_ALIAS } from '@/config/api';
...
export default {
    ...
    computed: {
        ...
        contentTitle() {
            ...
            const name = this.agentInfo.user.show_alias ? this.agentInfo.user.alias : this.agentInfo.user.username;
            let result = contentTitle.replace('%s', `<span class="${this.$style.account}"> ${name}</span>`);

            if (this.accountFieldOpen.config && this.accountFieldOpen.config.alias.display) {
    ...
    created() {
        ajax({
            ...
            if (response && response.result === 'ok') {
                this.accountFieldOpen = response.ret;
            }
        });
