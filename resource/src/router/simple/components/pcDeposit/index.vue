<template>
    <div :class="$style['simple-deposit-wrap']">
        <div :class="$style['header-wrap']">
            ...
            <div :class="$style['website-btn']" @click="goHome">{{ $text('S_HOME_ONE', '官网首页') }}</div>
        </div>
        <div :class="$style['content-wrap']">
            <div :class="$style['username-wrap']">
                <span>{{ $text('S_NAME', '会员帐号') }}</span>
                <span :class="$style['username-tip']">{{ $text('S_DEPOSIT_TIP01', '请确认您的帐号，款项存入后，即无法取消或变更帐号') }}</span>
            </div>
            ...
            <div v-if="isShowDeposit" :class="$style['deposit-wrap']">
                ...
                <div class="deposit-list-wrap">
                    <div
                        ...
                        :class="[$style['mode-item'], { [$style['is-current']]: curModeGroup.payment_group_id === listItem.payment_group_id }]"
                ...
                <div v-if="depositData.length > 0 && submitStatus === 'stepOne'" class="deposit-edit-wrap">
                    <template v-if="Object.keys(curModeGroup).length > 0">
                        <div v-if="!curModeGroup.uri" :class="$style['bank-card-wrap']">
                            <!-- 支付方式 -->
                            <div class="clearfix">
                                <p :class="$style['bank-card-title']">{{ $text('S_PAY_MODE', '支付方式') }}</p>
                                ...
                                <div :class="$style['pay-mode-item-wrap']">
                                    ...
                                    >
                                        ...
                                        <span :class="$style['pay-main-title']">{{ info.bank_name || info.payment_method_name }}</span>
                                        <span :class="$style['pay-sub-title']">{{ info.payment_type_name }}</span>
                                    </div>
                                </div>
                            </div>
                            <!-- 選擇銀行 or 選擇點卡 -->
                            <div v-if="curPayInfo.banks && curPayInfo.banks.length > 0" :class="[$style['feature-wrap'], 'clearfix']">
                                <span :class="$style['bank-card-title']">{{ curPayInfo.payment_method_id === 2 ? $text('S_SELECT_POINT_CARD_SHORT', '选择点卡') : $text('S_SELECT_BANKS_ONE', '选择银行') }}</span>
                            ...
                            <div v-if="!isDepositAi && passRoad.length > 0" :class="[$style['feature-wrap'], 'clearfix']">
                                <span :class="$style['bank-card-title']">{{ $text('S_PAY_PASS', '支付通道') }}</span>
                                <div :class="$style['bank-feature-wrap']">
                                    <div
                                        v-for="(item, index) in passRoad"
                                        :key="`pass-road-${index}`"
                                        :class="[$style['pay-mode-pass'], { [$style['is-current']]: curPassRoad.id === item.id }]"
                                        @click="changePassRoad(item)"
                                    >
                                        {{ item.mainTitle }}
                                    </div>
                                    <span :class="$style['pass-tip']">{{ $text('S_PAY_PASS_TIP', '若通道无法存款，请点选其他通道') }}</span>
                                </div>
                            </div>
                            <!-- 存款金額 -->
                            <div :class="[$style['feature-wrap'], 'clearfix']">
                                <span :class="$style['bank-card-title']">{{ $text('S_DEPOSIT_MONEY', '存款金额') }}</span>
                                <div :class="[$style['bank-feature-wrap']]">
                                    <div v-if="!isDepositAi && (Object.keys(curPassRoad).length === 0 || curPassRoad.is_custom_amount)" :class="$style['money-input-wrap']">
                                        <input
                                            v-model="moneyValue"
                                            :class="[$style['deposit-input'], { [$style['is-error']]: isErrorMoney && Number(depositInterval.maxMoney) }]"
                                            :placeholder="Number(depositInterval.maxMoney)
                                                ? $text(
                                                    'S_MONEY_RANGE', {
                                                        replace: [
                                                            {
                                        ...
                                        />
                                        <span v-if="isErrorMoney && Number(depositInterval.maxMoney)" :class="$style['error-tip']">
                                    ...
                                    <template v-if="isDepositAi || (curPassRoad && curPassRoad.is_recommend_amount)">
                                        <div
                            ...
                            <!-- 實際存入 -->
                            <div :class="[$style['feature-wrap'], 'clearfix']">
                                <span :class="$style['bank-card-title']">{{ $text('S_ESTIMATE_ENTER', '预计存入') }}</span>
                                <div :class="[$style['bank-feature-wrap']]">
                                    <span :class="$style['amount-text']">{{ realSaveMoney || 0.00 }}</span>
                                </div>
                            </div>
                            <!-- 匯款資料必填欄位 -->
                            <div>
                                <!-- 銀行匯款 極速到帳 or 支付轉帳，才需要輸入存款資料 -->
                                <div v-if="[5, 6].includes(curPayInfo.payment_type_id)" :class="$style['speed-input-wrap']">
                            ...
                            <div :class="$style['pay-button-wrap']">
                                <div
                                    :class="[$style['pay-button'], { [$style['is-speed']]: curPayInfo.payment_method_id === 6 }]"
                            ...
                            <!-- 溫馨提醒 -->
                            <div v-if="curModeGroup.payment_group_memo" :class="[$style['feature-wrap'], 'clearfix']">
                                <span :class="$style['bank-card-title']">{{ $text('S_TIPS', '温馨提示') }}</span>
                                <!-- eslint-disable vue/no-v-html -->
                        ...
                        <div v-else :class="$style['bank-card-wrap']">
                            ...
                            <div class="clearfix">
                                ...
                                <div :class="[$style['pay-mode-item'], $style['is-current']]">
                                    <img v-lazy="getImg({ bank_id: 'default' })" :class="$style['pay-mode-img']" />
                                    <span :class="$style['pay-third-main-title']">{{ curModeGroup.alias }}</span>
                                </div>
                            </div>
                            <div
        ...
        <div :class="$style['footer-wrap']">
            <span v-if="memInfo.config.domain_name">Copyright © {{ memInfo.config.domain_name[$i18n.locale] }} Reserved.</span>
        </div>
