...
export default {
  ...
  computed: {
    ...
    title() {
      return this.$text(
        this.isJpException
          ? "POP_NOTE_ACCOUNT_COMPLETE_2"
          : "POP_NOTE_ACCOUNT_COMPLETE",
        "请完成【我的帐户】设定"
      );
    },
    personalData() {
      const list = [
        ...
        {
          msg: this.$text(
            this.isJpException ? "S_POP_NOTE_LIST_1" : "POP_NOTE_LIST_1",
            "设定【个人姓名】"
          ),
        ...
        {
          msg: this.$text(
            this.isJpException ? "S_POP_NOTE_LIST_3" : "POP_NOTE_LIST_3",
            "设定【提现密码】"
          ),
          verifyFail: this.isJpException
            ? false
            : !this.memInfo.user.has_withdraw_password
        },
        // 手機號碼
        {
    ...
    bankCardData() {
      ...
      return {
        type: this.$text(
          this.isJpException ? "S_BIND_BANK_2" : "S_BIND_BANK",
          "绑定银行卡"
        ),
        list
      };
