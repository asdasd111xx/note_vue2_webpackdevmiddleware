        ...
        <div v-if="navData.navArr" class="nav clearfix">
            ...
            >
                ...
                <transition :name="animationType">
                    <div
                        v-if="item.sub_page && hoverNow === index"
                        :style="navSubWrapStyle(subPageMax(item.sub_page, item.pid))"
                        class="nav-sub-wrap"
                    >
                        <!-- eslint-disable vue/no-v-html -->
                        <div
                            v-if="navSubTitle[item.pid]"
                            class="nav-sub-title"
                            v-html="navSubTitle[item.pid][language]"
                        />
                        <!-- eslint-enable vue/no-v-html -->
                        <template v-for="(pageList, subPageIndex) in subPageMax(item.sub_page, item.pid)">
                            <div :key="`nav-sub-${subPageIndex}`" class="nav-sub-list">
                                <div
                                    v-for="subItem in pageList"
                    ...
                    >
                        ...
                        <transition :name="animationType">
                            <div
                                ...
                                :style="{ [moreDirection]: `-${subPageMax(moreItem.sub_page, moreItem.pid).length*moreListWidth + 4}px`, 'width': `${subPageMax(moreItem.sub_page, moreItem.pid).length*130 + 4}px`}"
                                class="nav-more-sub-wrap"
                            >
                                <template v-for="(pageList, subPageIndex) in subPageMax(moreItem.sub_page, moreItem.pid)">
                                    <div :key="`nav-more-sub-${subPageIndex}`" class="nav-more-sub-list">
                                        <div
                                            v-for="subItem in pageList"
...
export default {
    ...
    data() {
        return {
            ...
            navSubTitle: {
                19930: i18nList.navSubLink01,
                17715: i18nList.navSubLink02,
                17685: i18nList.navSubLink03,
                19533: i18nList.navSubLink04,
                17711: i18nList.navSubLink05
            }
        };
    },
    computed: {
    ...
    methods: {
        ...
        subPageMax(items, pid) {
            const pageArr = [];
            const pageList = cloneDeep(items);
            const count = pid === 17723 ? 5 : 8;
            items.forEach(() => {
                if (pageList.length === 0) {
                    return;
                }

                const resultArr = pageList.splice(0, count);
                pageArr.push(resultArr);
            });
        ...
        navSubWrapStyle(arr) {
            const styleData = {
                width: `${(arr.length * 130) + 4}px`
            };
            if (this.animationType) {
                styleData.left = `${-30 + ((arr.length - 1) * -60)}px`;
            }
