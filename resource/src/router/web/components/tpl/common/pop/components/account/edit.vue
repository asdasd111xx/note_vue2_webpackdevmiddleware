<template>
  <div>
    ...
    <div class="account-pop-btn" @click="edit(select.key)">
      {{ $t("S_JM_SURE_SEND") }}
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    edit(key) {
      ...
      if (key === "telegram") {
        re = /^[a-z0-9_、]{5,32}$/;
        msg = this.$text(
          "TELEGRAM_REGULAR_TEXT",
          "请输入5-32码(格式支援手机或Telegram Username)"
        );
      }
      if (key === "kakaotalk") {
        re = /^[A-Za-z0-9_.@]{4,100}$/;
        msg = this.$text(
          "KAKAOTALK_REGULAR_TEXT",
          "请至少输入4码(格式支援Email, 手机或 KakaoTalk ID)"
        );
      }
      ...
      apiSelect.accountDataSet({
        params: {
          [key]:
            this.select.key === "birthday"
              ? Vue.moment(this.selectValue).format()
              : this.selectValue
        },
        success: () => {
          alert(this.$t("S_CR_SUCCESS"));
