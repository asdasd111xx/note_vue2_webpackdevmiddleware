    ...
    <transition name="fade">
        <div class="pop-note-warp pop-class">
            ...
            <div :class="['content', siteConfig.JAPAN_EXCEPTION === 'Y' && path[0] === 'mobile' ? 'left' : '']">
                <template>
                    <p v-if="siteConfig.JAPAN_EXCEPTION === 'Y' && position === 'member'">{{ $text('POP_NOTE_ACCOUNT_COMPLETE', '请完成【我的帐户】设定') }}：</p>
                    <p v-else>
                        {{ $text('POP_NOTE_ACCOUNT_COMPLETE', '请完成【我的帐户】设定') }}：
                    </p>
                </template>
                <div
                    v-if="hasAccountData"
    ...
    computed: {
        ...
        noteMsg() {
            return [
                ...
                {
                    text: this.$text(this.isException ? 'S_POP_NOTE_LIST_4' : 'S_POP_NOTE_LIST_4', '验证【手机号码】'),
                    isShow: !this.mobileCheck ? !this.mobileCheck : false
                }
            ].filter((item) => item.isShow);
        },
        bankCardMsg() {
            const isMember = this.position === 'member';

            return [
                {
                    text: this.isException
                        ? this.$text('POP_NOTE_NUMBER_COMPLETE', '请完善【常用帐号】信息')
                        : this.$text(isMember ? 'POP_NOTE_NUMBER_COMPLETE' : 'POP_NOTE_NUMBER_COMPLETE', {
                            text: isMember ? '请完善【常用帐号】信息' : '请完善【常用帐号】信息'
                        }),
