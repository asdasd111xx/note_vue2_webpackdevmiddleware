...
export default page =>
  axios.all([store.dispatch("actionSetUserdata")]).then(() => {
    ...
    if (
      store.state.memInfo.user.bankrupt === "1" &&
      store.state.mcenterpage !== page &&
      (page === "accountBankCard" ||
        page === "deposit" ||
        page === "withdraw" ||
        page === "bankBalanceTran")
    ) {
    ...
    if (
      ((page === "accountBankCard" || page === "withdraw") &&
        !store.state.memInfo.user.name) ||
      (page === "withdraw" &&
        (!(isException() || store.state.memInfo.user.has_withdraw_password) ||
          !store.state.mobileCheck))
    ) {
