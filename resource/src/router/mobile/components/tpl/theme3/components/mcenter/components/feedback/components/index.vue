    ...
    <mobile-container class="theme-basic">
        <div slot="content" class="content-main">
            ...
            <feedback :select-type-id.sync="currentSelectId">
                <template scope="{ currentTab, fieldList, paramsData, goImageRelease, stepText, submitData, errorField, setInputValue }">
                    <div v-if="!selectType" class="feedback-wrap">
                        <div class="feedback-title-wrap">{{ $text('S_ASK_QUESTION_TYPE', '请选择您要咨询的问题类别') }}</div>
                    ...
                    <div v-else class="feedback-ask-wrap">
                        ...
                        <div class="ask-content-wrap">
                            <div class="ask-title">
                                ...
                                <span class="sub-text">（{{ $text('S_WORD_COUNT', '字数上限1000字') }}）</span>
                            </div>
                            <textarea
                                :value="paramsData.content"
                                :placeholder="$text('S_DESCRIBE_QUESTION_TIP', '请详细描述您遇到的问题，字数上限1000字')"
                                maxlength="1000"
                                class="field-textarea"
                                @input="setInputValue($event.target.value, 'content')"
                            />
                        </div>
                        <div class="image-release-btn" @click="goImageRelease">{{ $text('S_IMAGE_RELEASE', '上传图片') }}</div>
                        ...
                        <div class="feedback-btn" @click="submitData">{{ $text('S_SUBMIT', '提交') }}</div>
                    </div>
                </template>
            </feedback>
        </div>
