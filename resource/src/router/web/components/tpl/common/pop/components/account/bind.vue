<template>
    <div :class="`${!select.verification ? 'no-verification' : ''} account-pop-bind`">
        <div v-if="select.key==='email'" :class="`${!infoSelect && !select.verification ? 'alone' : ''} account-pop-text`">
            {{ $t('S_SECURITY_LEVEL') }}
        </div>
        <div :class="`${!infoSelect && select.key==='phone' ? 'no-radio' : ''} clearfix`">
            <!-- 填寫舊資料欄位(原信箱有資料且狀態為already才會顯示) -->
            <select
                v-if="infoSelect && select.status === 'already' && select.key === 'phone'"
            ...
            <div v-if="infoSelect && select.status === 'already'" class="account-pop-field main-input">
                <input
                    v-model="bindOld"
                    :placeholder="`${select.key === 'email' ? $t('S_ORIGINAL_EMAIL') : $t('S_ORIGINAL_PHONE') }`"
                    type="text"
                />
            </div>
            <select
            ...
            <div :class="`${select.verification ? 'verification' : ''} account-pop-field main-input`">
                <input
                    v-model="bindNew"
                    :placeholder="selectNewText"
                    type="text"
                />
            </div>
            <!-- 選擇短信或語音(電話才會顯示) 先隱藏 -->
        ...
        <div class="account-pop-btn" @click="select.verification ? bindCheck(select.key) : setUserData(select.key)">{{ $t('S_JM_SURE_SEND') }}</div>
    </div>
</template>
...
export default {
    ...
    computed: {
        ...
        checkCodeText() {
            // 目前驗證碼時間為 5 分鐘
            return this.$t('S_SEND_CHECK_CODE_VALID_TIME').replace('%s', 5);
        }
    },
    created() {
